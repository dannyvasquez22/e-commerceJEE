					...
					<div class="features_items"><!--features_items-->
						...
						<%
							if(Integer.parseInt(session.getAttribute("category").toString()) > 0) {
								...
								datos = ProductoCad.listarProductosPorMarca(session.getAttribute("brand").toString(), Integer.parseInt(session.getAttribute("brand").toString()));
							} else {
							...
							<div class="product-image-wrapper">
								...
								<div class="choose">
									<ul class="nav nav-pills nav-justified">
										...
										<li>
											<a href="#" <c:if test="${p.stock == 0}"> class="gris" </c:if>>
											<c:choose>
												<c:when test="${p.stock > 0}">
													<i class="fa fa-check-circle"></i> (<c:out value="${p.stock}"> </c:out>) Disponible	
												</c:when>
												<c:otherwise>
													<i class="fa fa-check-lock"></i> (<c:out value="${p.stock}"> </c:out>) Agotado
												</c:otherwise>
											</c:choose>
